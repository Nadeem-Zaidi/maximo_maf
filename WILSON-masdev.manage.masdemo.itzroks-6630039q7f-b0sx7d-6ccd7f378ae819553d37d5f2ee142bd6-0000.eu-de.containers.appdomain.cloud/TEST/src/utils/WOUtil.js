      ...
      if (assetMeterDs && assetMeterDs.items.length > 0) {
        for (let i = 0; i < assetMeterDs.items.length; i++) {
          ...
          if (typeof udpatedMeterObj.newreading !== "undefined") {
            ...
            // istanbul ignore else
            if (udpatedMeterObj.newreading === 0 || udpatedMeterObj.newreading) {
              let newmeterreading = {
      ...
      if (locationMeterDs && locationMeterDs.items.length > 0) {
        for (let j = 0; j < locationMeterDs.items.length; j++) {
          ...
          if (typeof udpatedLocMeterObj.newreading !== "undefined") {
            ...
            // istanbul ignore else
            if (udpatedLocMeterObj.newreading === 0 || udpatedLocMeterObj.newreading) {
...
const validateMeterDate = (app, page, datasource, callEnableDisable = true) => {
  ...
  if (page.state.drawerName !== "update_multiMeterReading_drawer") {  
    if (assetMeterDs && assetMeterDs.items) {
      hasAssetAnyNewReading = assetMeterDs.items.some((item) => (item.newreading === 0 || item.newreading)); // DT239591 Gauge meter reading entries unable to accept 0 (zero) as a valid value on UI
    }
    
    if (locationMeterDs && locationMeterDs.items) {
      hasLocationAnyNewReading = locationMeterDs.items.some((item) => (item.newreading === 0 || item.newreading)); // DT239591 Gauge meter reading entries unable to accept 0 (zero) as a valid value on UI
    }
  ...
  if (assetMeterds.name === 'woMultiAssetLocationds') {
    ...
    if (page.state.assetMeterData.items.length > 0) {
      assetMeterNewReading = page.state.assetMeterData.items.some((item) => (item.newreading === 0 || item.newreading)); // DT239591 Gauge meter reading entries unable to accept 0 (zero) as a valid value on UI
    }
    
    // istanbul ignore if
    if (page.state.locationMeterData.items.length > 0) {
      locationMeterNewReading = page.state.locationMeterData.items.some((item) => (item.newreading === 0 || item.newreading)); // DT239591 Gauge meter reading entries unable to accept 0 (zero) as a valid value on UI
    }
