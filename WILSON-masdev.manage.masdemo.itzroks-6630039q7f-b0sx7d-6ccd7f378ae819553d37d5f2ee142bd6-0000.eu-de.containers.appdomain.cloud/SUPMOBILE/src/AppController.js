...
class AppController {
  applicationInitialized(app) {
    ...
    this.app.on('page-changed', (nextPage, prevPage) => {
      if (prevPage.name === 'workOrderDetails' && nextPage.name === 'approvals' && nextPage.state.previousPage === 'schedulecardlist' && nextPage.state.mapOriginPage === 'wodetail') {
   ...
   */
  async getOfflineWoStatusList(evt) {
    let statusArr = [];
  ...
  /**
   * It's used to switch to part identifier app
   * @param {*} args 
   */
  async openPartIdentifier(args) {
    const switcher = AppSwitcher.get();
    const context = args.context ? args.context : {};
    const options = args.options ? args.options : { canReturn: true };

    // This is to fix the circular structure error when persisting application states
    this.app.pages.find(x=>x.name==="approvals").state.signatureDs=undefined;

    this.app.state.incomingContext = null;
    const breadcrumbData = {
      returnName: `Returning to ${this.app.name}`, 
      enableReturnBreadcrumb: true
    };
    context.breadcrumb = breadcrumbData;
    switcher.discardTransition();
    switcher.gotoApplication('humai', context, options);
  }
