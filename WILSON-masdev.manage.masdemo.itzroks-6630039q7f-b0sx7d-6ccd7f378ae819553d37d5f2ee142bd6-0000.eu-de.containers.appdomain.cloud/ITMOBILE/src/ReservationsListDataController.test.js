 ...
 */
import ReservationsListDataController from "./ReservationsListDataController";
...
it("Datasource initialized", async () => {
  const controller = new ReservationsListDataController();
...
it("Load data and execute _computeDueDate functions", async () => {
  const controller = new ReservationsListDataController();
  const app = new Application();
  const reservationsListDS = newDatasource(
    reservationsData,
    "reservationsListDS"
  );
